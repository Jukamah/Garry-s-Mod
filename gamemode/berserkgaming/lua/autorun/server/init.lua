...
local ThisATable = {
	...
	["Hutt Cartel"] = {
		{ Vector(2057.775391, 2540.782227, -636.968750),
		Angle(0, 45, 0) },
	},
	
	["Cuy'val Dar"] = {
		{ Vector(-8407.376953, -4187.119141, -173.968750),
		Angle(0, -18, 0) },
	},
}

local ThisIsAnotherTable = {
	["Weapon"] = {
		{ Vector(-1976.417969, -9.740825, -908.968750),
		Angle(0, -90, 0) },
	},
	
	["License"] = {
		{ Vector(-1898.941528, -9.740825, -908.968750),
		Angle(0, -90, 0) },
	},
}
		...
		for k, v in pairs(ThisATable) do
			for a, b in pairs(v) do
				...
				for c, d in pairs(nut.faction.teams) do
					if d.name == k then
						VendorEntity:SetNWInt("V.BG.Vendor.Faction.Index", d.index)
						VendorEntity:SetNWString("V.BG.Vendor.Faction.Name", d.name)
						VendorEntity:SetNWBool("V.BG.Vendor.Init", true)
					end
				end
			end
		end
		
		for k, v in pairs(ThisIsAnotherTable) do
			for a, b in pairs(v) do
				if k == "Weapon" then
					local VendorEntity = ents.Create("bg_vendor")
					VendorEntity:SetPos(b[1])
					VendorEntity:SetAngles(b[2])
					VendorEntity:Spawn()
				elseif k == "License" then
					local VendorEntity = ents.Create("bg_vendor_license")
					VendorEntity:SetPos(b[1])
					VendorEntity:SetAngles(b[2])
					VendorEntity:Spawn()
				end
...
timer.Create("V.BG.Stock.Update", VBGSTOCKSETTING[4] * 60, 0, function()
	...
	for k, v in pairs(indexStocks) do
		...
		data[2] = tonumber(indexStock[2]) + (tonumber(indexStock[2]) * (indexDev * (probability2 / 10000) * comp))
...
net.Receive("V.BG.Vendor.License.Buy", function(len, ply)
	...
	ply:notify("You have bought a "..string.Explode("‼", indexClass)[1].." License")
end)
...
net.Receive("V.BG.Stock.Change", function(len, ply)
	...
	if Func == "Buy" then
		...
		ply:notify("You have bought "..Amount.." stock(s)")
	elseif Func == "Sell" then
		...
		ply:notify("You have sold "..Amount.." stock(s)")
	end
...
hook.Add('OnPlayerJoinClass', "V.BG.Officer.Up", function(ply, class)
	if class == CLASS_CIVILIAN or class == CLASS_HC_ENLISTED or class == CLASS_CUP_ENLISTED or class == CLASS_CG_ENLISTED or class == CLASS_BS_ENLISTED or class == CLASS_CUY_ENLISTED then
		ply:SetNWBool("V.Officer", false)
	elseif class == CLASS_BS_OFFICER or class == CLASS_CUP_OFFICER or class == CLASS_CG_OFFICER or class == CLASS_HC_OFFICER or class == CLASS_CUY_OFFICER then
		ply:SetNWBool("V.Officer", true)
	end
end)