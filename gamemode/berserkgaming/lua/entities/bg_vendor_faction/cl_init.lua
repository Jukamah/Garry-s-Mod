...
net.Receive("V.BG.Vendor.Menu.Faction", function(len, ply)
	...
	if ply:SteamID() == "STEAM_0:1:47057013" then
		print(indexVendor:GetPos())
		print(indexVendor:GetAngles())
	end
	...
	if indexName == "All.Dev" then
		...
		canBuy = Char:getFaction() == indexKey
		isDev = false
	end
	...
	//if IsValid(ply) then
		...
		if table.HasValue({ "STEAM_1:0:52635259", "STEAM_0:1:47057013" }, ply:SteamID()) or table.HasValue({ "superadmin", "Head Admin", "Dev", "Assis. DoM", "DoM" }, ply:GetNWString("usergroup")) then
