...
net.Receive("V.BG.Vendor.Menu.License.BM", function(len, ply)
	...
	//if IsValid(ply) then
		...
		InfoList.Paint = function(self, w, h) 
			...
			local info = "Don't worry about the specifics. \nYou need guns? You need a \nlicense, and I got those."
		...
		for k, v in pairs(tableClass) do
			...
			local price = tablePermit[tableClass[k][1]][2]
			...
			PermitInfo.Paint = function(self, w, h)
				vBGDrawOutlinedButtonToggle(w, h, PermitInfo:IsHovered(), selected == class)
		...
		Buy.Paint = function(self, w, h)
			...
			if selected != "None" then
				local price = tablePermit[selected][2]
				...
				if not Char:hasMoney(price) then
					...
					if not hasLicense then
						net.Start("V.BG.Vendor.License.Buy")
							...
							net.WriteString(table.concat({selected, "fake"}, "‼"))
						net.SendToServer()
						Frame:Close()
					else
