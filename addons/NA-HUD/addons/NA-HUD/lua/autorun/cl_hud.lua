if CLIENT then
	...
	settings[4] = true			// True = Weapon Selection Slides, False = Appears/Dissapears
	...
	local fakeNames = { "weapon_ttt_defuser", "weapon_ttt_cse", "weapon_ttt_health_station", "weapon_ttt_binoculars", "weapon_ttt_stungun", "weapon_ttt_c4", "weapon_ttt_radio", "weapon_ttt_flaregun", "weapon_ttt_phammer", "weapon_ttt_sipistol", "weapon_ttt_push", "weapon_ttt_wtester", "weapon_zm_improvised", "weapon_ttt_unarmed", "weapon_zm_carry", "weapon_ttt_knife", "weapon_ttt_teleport" }

	// THE NAME THAT YOU WANT
	local realNames = { "Defuser", "Visualizer", "Health Station", "Binoculars", "UMP Prototype", "C4", "Radio", "Flare Gun", "Poltergeist", "Silenced Pistol", "Newton Launcher", "Radar", "Crowbar", "Holstered", "Magneto-stick", "Knife", "Teleporter" }
	...
	function drawSel()
		for i = 0, #(LocalPlayer():GetWeapons()) - 1, 1 do
			drawCell(i, LocalPlayer():GetWeapons()[i + 1], cur, offset)
		end
		
		if settings[4] then
			if change then
				if offset > 0 then
					offset = offset - (ScrW() * 0.00000625)
				end
			else
				if offset < 1 then
					offset = offset + (ScrW() * 0.00000625)
	...
	function drawCell(index, weapon, highlight, offset)
		xOffset = offset * (ScrW() * 0.078125)
		...
		surface.DrawRect(xOffset + (ScrW() * 0.90625), (ScrH() * 0.8722222222222222) - (index * (ScrH() * 0.0555555555555556)), (ScrW() * 0.084375), (ScrH() * 0.0288888888888889))
		draw.Circle((xOffset + ScrW() * 0.990625), ((ScrH() * 0.8861111111111111) - (index * (ScrH() * 0.0555555555555556))), (ScrW() * 0.0078125), 15)
		
		surface.SetDrawColor(100, 100, 100)
		surface.DrawRect(xOffset + (ScrW() * 0.90625), ((ScrH() * 0.8755555555555556) - (index * (ScrH() * 0.0555555555555556))), (ScrW() * 0.08125), (ScrH() * 0.0222222222222222))
		
		surface.SetDrawColor(100, 100, 255)
		draw.Circle(xOffset + (ScrW() * 0.90625), ((ScrH() * 0.8861111111111111) - (index * (ScrH() * 0.0555555555555556))), (ScrW() * 0.0125), 5)
		
		draw.SimpleText((index + 1), "NA-HUD-SEL", xOffset + (ScrW() * 0.905625), (ScrH() * 0.8861111111111111) - (index * (ScrH() * 0.0555555555555556)), colour[9], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(getWeaponName(weapon:GetClass()), "NA-HUD-SEL", xOffset + (ScrW() * 0.91875), (ScrH() * 0.8862) - (index * (ScrH() * 0.0555555555555556)), colour[9], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
